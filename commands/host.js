...
module.exports = {
	...
	async execute(interaction) {
		...
		if(true){
			...
			//verify hosting user is registered to do gibs
			axios.get("http://localhost:8080/api/wallets")
				.then((res)=>{
					...
					if(found) {
						...
						interaction.editReply(`you are not registered`);
						replied = true;
					}
				})
				.catch((err)=>{
					console.log(err);
					interaction.editReply('failed to verify registered user')
					replied = true
				})

			//verify axie was sent from donor wallet to gib wallet
			//ensures only the axie donator can host the gibaway
				...
				.then((res)=>{
					if ('transfers' in res.data && res.data.transfers.length > 0){
						if (res.data.transfers[0].to == gibWallet && res.data.transfers[0].from == donorWallet){
							...
							if (!replied) {interaction.editReply('host failed.\n\n If the gibaway axie was recently transferred, try waiting a few minutes\n\n otherwise, double check the data you provided')}
						}
					}
					else{
						interaction.editReply('axie not valid, try waiting')
					}
				})
				.catch((err)=>{
					console.log(err);
					if (!replied) {interaction.editReply('failed to verify2')}
				})
		}
	}
};
